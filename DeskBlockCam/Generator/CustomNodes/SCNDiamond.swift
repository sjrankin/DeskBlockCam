...
class SCNDiamond: SCNNode
{
    ...
    public static func Geometry(MajorAxis: CGFloat, MinorAxis: CGFloat, Height: CGFloat) -> SCNGeometry
    {
        ...
        Path.line(to: CGPoint(x: MinorAxis, y: MajorAxis / 2.0))
        Path.line(to: CGPoint(x: MinorAxis / 2.0, y: MajorAxis))
        Path.line(to: CGPoint(x: 0.0, y: MajorAxis / 2.0))
        Path.line(to: CGPoint(x: MinorAxis / 2.0, y: 0.0))
        Path.close()
        #else
