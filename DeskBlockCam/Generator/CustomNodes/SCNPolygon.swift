...
class SCNPolygon: SCNNode
{
    ...
    init(VertexCount: Int, Radius: CGFloat, Depth: CGFloat)
    {
        ...
        if VertexCount < 3
        {
            fatalError("Vertex count must be 3 or greater.")
        }
        if Radius <= 0.0
        {
            fatalError("Radius must be greater than 0.0.")
        }
        self.Radius = Radius
    ...
    {
        self.geometry = SCNPolygon.Geometry(VertexCount: _VertexCount, Radius: _Radius, Depth: _Depth)
    }
